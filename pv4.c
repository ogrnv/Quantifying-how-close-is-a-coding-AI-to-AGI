/*
Please change TTSR function in the following C program TTSR.c to achieve the best test results i.e minimal number of moves in all tests of pv4c:
gcc pv4.c TTSR.c -o pv4c
./pv4c
See the definitions of moves, test... below in the pv4.c program.

It is necessary to exclude the occurrence of any software loops, for example, when a chip moves along a closed path due to changing of the TTSR function.
...
unsigned char v[4], CT[16][16], N5[5];
uint32_t dat[16];
void TTSR(unsigned char v[4], uint32_t dat[16], unsigned char CT[16][16],unsigned char N5[5]);
...
dat[0]=tsts=atoi(argv[1]);
dat[1]=rounds=atoi(argv[2]); //the number of rounds in 1 test up to rmax
dat[2]=stp=atoi(argv[3]);
dat[3]=hm=atoi(argv[4]);
dat[4]=kcb=atoi(argv[5]);
dat[5]=nub=atoi(argv[6]);
dat[6]=dbg=atoi(argv[7]);
...
for (int i = 0; i < hm; i += 1){
	for (int j = 0; j < hm; j += 1){
		CT[i][j]=Csb[i][j];
	}
}
for (int i = 0; i < 5; i += 1){N5[i]=n5[i];} 
//**********************

TTSR(v, dat, CT, N5); //TTSR see below in TTSR.c
//TTSR IS A TEST TASKS SOLVER for setting x0,y00 and x1,y01 i.e. SA&FA
x0=v[0]; y00=v[1]; x1=v[2];  y01=v[3];
//if(dbg==1) printf("ttsr %d %d %d %d %f\n",x0,y00,x1,y01,mn);
//**********************
...
uint32_t random(uint32_t min, uint32_t max) {
	...
	//Next two lines are a solver for hm=8 based on the Monte Carlo method:
	
	//v[0]=random(0, 7); v[1]=random(0, 7); //x0,y00 i.e SA
	//v[2]=random(0, 7); v[3]=random(0, 7); //x1,y01 i.e. FA
}
*/
